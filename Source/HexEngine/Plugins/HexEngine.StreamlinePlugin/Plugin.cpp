...
CREATE_PLUGIN(g_pStreamlinePlugin, StreamlinePlugin);

StreamlinePlugin::StreamlinePlugin()
{
...
/// </summary>
void StreamlinePlugin::Destroy()
{
	SAFE_DELETE(_streamline);
}
...
HexEngine::IPluginInterface* StreamlinePlugin::CreateInterface(const std::string& interfaceName)
{
	...
	//
	if (interfaceName == HexEngine::IStreamlineProvider::InterfaceName)
		return _streamline;
