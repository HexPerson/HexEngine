...
CREATE_PLUGIN(g_pAngelScriptPlugin, AngelScriptPlugin);

AngelScriptPlugin::AngelScriptPlugin()
{
	_interface = new AngelScript;
}
...
/// </summary>
void AngelScriptPlugin::Destroy()
{
...
void AngelScriptPlugin::GetVersionData(VersionData* data)
{
...
IPluginInterface* AngelScriptPlugin::CreateInterface(const std::string& interfaceName)
{
	...
	//
	if (interfaceName == AngelScript::InterfaceName)
		return _interface;

	return nullptr;
}

void AngelScriptPlugin::GetDependencies(std::vector<std::string>& dependencies) const
{
