...
#include <unordered_set>
		...
		int32_t yawQuarterTurns)
	{
		...
		for (const auto& action : actions)
		{
			...
			auto* wrapper = scene->CreateEntity(MakeManagedWrapperName(action.coord), GridToWorld(action.coord, straightSpec.length, action.height), action.placement.rotation, math::Vector3(1.0f));
			if (wrapper == nullptr)
				continue;
			...
			if (action.placement.isPrefab)
				...
				SpawnMeshCell(wrapper, action.placement.assetPath);
...
void CitySimulationEditorToolPlugin::PaintOrthogonalRun(const math::Vector3& worldPosition)
{
	...
	ApplyIncrementalRoadNetworkChange(scene, runCells, _roadPainterAnchorHeight, straightSpec, cornerSpec, crossroadSpec, tJunctionSpec, _roadPainterYawQuarterTurns);
