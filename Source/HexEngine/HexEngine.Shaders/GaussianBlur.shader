...
"PixelShader"
{
	...
	float4 ShaderMain(UIPixelInput input) : SV_Target
	{
        ...
        for (int i = 1; i < 5; ++i)
        {
			float4 result1 = shaderTexture.Sample(PointSampler, input.texcoord.xy + float2(tex_offset.x * i, 0.0));

			if (length(result1.rgb) > 0.0f)
				result += result1.rgb * weight[i];

			float4 result2 = shaderTexture.Sample(PointSampler, input.texcoord.xy - float2(tex_offset.x * i, 0.0));
