...
"PixelShader"
{
    ...
    float4 ShaderMain(UIPixelInput input) : SV_Target 
    {
        ...
        float2 rcpFrame = float2(1.0f / (float)g_screenWidth, 1.0f / (float)g_screenHeight);// *5.0f;

        float2 pos = input.texcoord;// float2(input.position.x / (float)g_screenWidth, input.position.y / (float)g_screenHeight);

        FxaaTex tex;
        tex.tex = g_inputTex;
        tex.smpl = g_pointSampler;
