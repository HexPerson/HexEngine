...
"PixelShader"
{
    ...
    float4 ShaderMain(UIPixelInput input) : SV_Target
    {
        ...
        float2 tex_offset = 1.0 / float2(g_screenWidth / 2, g_screenHeight / 2); // gets size of single texel
        ...
        {
            result += shaderTexture.Sample(PointSampler, input.texcoord.xy + float2(0.0, tex_offset.y * i)).rgb * weight[i];
            result += shaderTexture.Sample(PointSampler, input.texcoord.xy - float2(0.0, tex_offset.y * i)).rgb * weight[i];
        }
