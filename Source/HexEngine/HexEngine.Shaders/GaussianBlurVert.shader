...
"PixelShader"
{
    ...
    float4 ShaderMain(UIPixelInput input) : SV_Target
    {
       ...
       //     return float4(colour.rgb, 0.0f);
        ...
        for (int i = 1; i < 5; ++i)
        {
            float4 result1 = shaderTexture.Sample(PointSampler, input.texcoord.xy + float2(0.0, tex_offset.y * i * 2));

            //if (length(result1.rgb) > 0.0f)
                result += result1 * weight[i];

            float4 result2 = shaderTexture.Sample(PointSampler, input.texcoord.xy - float2(0.0, tex_offset.y * i * 2));
