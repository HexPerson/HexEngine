...
namespace HexEngine
{
	...
	PlaceOfWorkComponent::PlaceOfWorkComponent(Entity* entity) :
	...
	PlaceOfWorkComponent::PlaceOfWorkComponent(Entity* entity, PlaceOfWorkComponent* copy) :
		BaseComponent(entity)
	{
		if (copy != nullptr)
		{
	...
	void PlaceOfWorkComponent::Serialize(json& data, JsonFile* file)
	{
	...
	void PlaceOfWorkComponent::Deserialize(json& data, JsonFile* file, uint32_t mask)
	{
		(void)mask;
	...
	bool PlaceOfWorkComponent::CreateWidget(ComponentWidget* widget)
	{
		auto* roles = new LineEdit(widget, widget->GetNextPos(), Point(widget->GetSize().x - 20, 18), L"Role Tags (CSV)");
		roles->SetValue(std::wstring(_roleTagsCsv.begin(), _roleTagsCsv.end()));
		roles->SetDoesCallbackWaitForReturn(false);
		roles->SetOnInputFn([this](LineEdit* edit, const std::wstring& value)
		{
			_roleTagsCsv = std::string(value.begin(), value.end());
			RebuildRoleTags();
		});

		auto* capacity = new DragInt(widget, widget->GetNextPos(), Point(widget->GetSize().x - 20, 18), L"Worker Capacity", &_workerCapacity, 1, 100000, 1);
		auto* priority = new DragInt(widget, widget->GetNextPos(), Point(widget->GetSize().x - 20, 18), L"Service Priority", &_servicePriority, -100, 100, 1);

		auto* shifts = new LineEdit(widget, widget->GetNextPos(), Point(widget->GetSize().x - 20, 18), L"Shift Windows (CSV)");
		shifts->SetValue(std::wstring(_shiftWindowsCsv.begin(), _shiftWindowsCsv.end()));
		shifts->SetDoesCallbackWaitForReturn(false);
		shifts->SetOnInputFn([this](LineEdit* edit, const std::wstring& value)
		{
			_shiftWindowsCsv = std::string(value.begin(), value.end());
			RebuildShiftsFromCsv();
		});
