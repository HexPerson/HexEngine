...
namespace HexEngine
{
	CityEmergencyDispatcherSystemComponent::CityEmergencyDispatcherSystemComponent(Entity* entity) :
		UpdateComponent(entity)
	{
		RegisterEntityListener();
	}

	CityEmergencyDispatcherSystemComponent::CityEmergencyDispatcherSystemComponent(Entity* entity, CityEmergencyDispatcherSystemComponent* copy) :
	...
	void CityEmergencyDispatcherSystemComponent::Serialize(json& data, JsonFile* file)
	{
		file->Serialize(data, "_enabled", _enabled);
		file->Serialize(data, "_dispatchTickSeconds", _dispatchTickSeconds);
	}

	void CityEmergencyDispatcherSystemComponent::Deserialize(json& data, JsonFile* file, uint32_t mask)
	{
		(void)mask;
	...
	bool CityEmergencyDispatcherSystemComponent::CreateWidget(ComponentWidget* widget)
	{
		auto* enabled = new Checkbox(widget, widget->GetNextPos(), Point(widget->GetSize().x - 20, 18), L"Enabled", &_enabled);
		auto* dispatchTick = new DragFloat(widget, widget->GetNextPos(), Point(widget->GetSize().x - 20, 18), L"Dispatch Tick", &_dispatchTickSeconds, 0.05f, 60.0f, 0.05f, 2);
		...
		new Button(widget, widget->GetNextPos(), Point(widget->GetSize().x - 20, 18), L"Report Medical Incident", [this](Button* button) -> bool
		{
			const math::Vector3 aroundSystem = GetEntity()->GetWorldTM().Translation();
			const math::Vector3 offset(GetRandomFloat(-1000.0f, 1000.0f), 0.0f, GetRandomFloat(-1000.0f, 1000.0f));
	...
	void CityEmergencyDispatcherSystemComponent::OnRemoveEntity(Entity* entity)
	{
		if (entity == nullptr)
			return;
	...
	uint32_t CityEmergencyDispatcherSystemComponent::ReportEmergency(const math::Vector3& worldPosition, const std::string& requiredServiceTag)
	{
		...
		incident.createdAtTime = g_pEnv->_timeManager != nullptr ? g_pEnv->_timeManager->_currentTime : 0.0f;
	...
	RoutineAgentComponent* CityEmergencyDispatcherSystemComponent::FindBestEmergencyResponder(const EmergencyIncident& incident, const ServiceStationComponent* station) const
	{
		...
		for (auto* agent : agents)
		{
			...
			const uint32_t tie = CRC32::HashString(agent->GetEntity()->GetName().c_str());
