...
namespace HexEngine
{
	...
	class HEX_API CityRoutineSystemComponent : public UpdateComponent, public IEntityListener, public ITaskTravelService
	{
	public:
		...
		virtual bool EnqueueTask(Entity* agentEntity, const RoutineTaskSpec& spec) override;
		virtual bool RequestTravel(Entity* agentEntity, const std::string& destinationAnchorWaypointEntityName, RoutineTravelMode travelMode = RoutineTravelMode::DriveFirst) override;
		virtual bool CancelTask(Entity* agentEntity, const std::string& reason) override;
	...
	private:
		...
		Entity* AcquireVehicleForTask(RoutineAgentComponent* agent, RoutineTaskSpec& task);
		bool ConfigureVehicleRoute(Entity* vehicleEntity, RoutineAgentComponent* agent, const RoutineTaskSpec& task);
