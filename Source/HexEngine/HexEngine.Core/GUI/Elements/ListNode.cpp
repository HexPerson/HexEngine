...
namespace HexEngine
{
	...
	void ListNode::Render(GuiRenderer* renderer, const Point& position, const Point& size)
	{
		if (_child)
		{
			renderer->PushFillTexturedQuad(_arrowIcon, /*_list->GetPosition().x +*/ _list->GetSize().x - TreeListLineHeight, position.y, TreeListLineHeight, TreeListLineHeight, math::Color(HEX_RGBA_TO_FLOAT4(100, 100, 100, 255)), _isOpen ? 0.0f : 90.0f);
		}
		
		renderer->PushFillTexturedQuad(GetIcon(), position.x + 4, position.y + 1, 16, 16, math::Color(1, 1, 1, 1));

		renderer->PushPrintText(renderer->_style.font, (uint8_t)Style::FontSize::Tiny, position.x + 24, position.y + TreeListLineHeight / 2, renderer->_style.text_regular, FontAlign::CentreUD, GetLabel());
	}
