	...
	void LineEdit::Render(GuiRenderer* renderer, uint32_t w, uint32_t h)
	{
		...
		if (_canvas.BeginDraw(renderer, w, h))
		{
			...
			if ((IsMouseOver(true) || _hasInputFocus) && _label.length() > 0)
				renderer->PrintText(renderer->_style.font.get(), (uint8_t)Style::FontSize::Tiny, 0, 0 + _size.y / 2, renderer->_style.text_highlight, FontAlign::CentreUD, _label);
			else
				renderer->PrintText(renderer->_style.font.get(), (uint8_t)Style::FontSize::Tiny, 0, 0 + _size.y / 2, renderer->_style.text_regular, FontAlign::CentreUD, _label);

			int32_t boxX = 0 + minSize;
			int32_t boxWidth = std::max(0, _size.x - minSize);
			renderer->FillQuad(boxX, 0, boxWidth, _size.y, renderer->_style.lineedit_back);
			renderer->Frame(boxX, 0, boxWidth, _size.y, 1, renderer->_style.win_border);

			_hovering = IsMouseOver(boxX, 0, boxWidth, _size.y);
			...
			if (_icon)
			{
				...
				renderer->FillTexturedQuad(_icon.get(), textX, 0 + 3, iconSize, iconSize, _iconColour);
				textX += iconSize + 4;
			...
			if (_hasInputFocus)
			{
				renderer->FillQuad(textX + prefixWidth + prefixGap + valueWidth, 0 + 2, 2, _size.y - 4, renderer->_style.text_regular);
			}
