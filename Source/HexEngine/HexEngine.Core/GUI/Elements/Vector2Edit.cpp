...
namespace HexEngine
{
	...
	Vector2Edit::Vector2Edit(Element* parent, const Point& position, const Point& size, const std::wstring& label, math::Vector2* vector, std::function<void(const math::Vector2&)> callback) :
		...
		_callback(callback)
	{
		...
		for (int i = 0; i < 2; ++i)
		{
			...
			_lineEdits[i]->SetOnInputFn(std::bind(&Vector2Edit::OnSetAxisValue, this, i, std::placeholders::_1, std::placeholders::_2));
	...
	void Vector2Edit::OnSetAxisValue(int32_t axis, LineEdit* edit, const std::wstring& value)
	{
		if (axis >= 0 && axis <= 1)
		{
			math::Vector2 changedValue = *_vector;
	...
	void Vector2Edit::OnSetAxisValueFloat(int32_t axis, float value)
	{
		if (axis >= 0 && axis <= 1)
		{
			math::Vector2 changedValue = *_vector;
	...
	void Vector2Edit::SetAxisLabels(const std::wstring labels[3])
	{
		for (int i = 0; i < 2; ++i)
			_axisLabels[i] = labels[i];
	}

	void Vector2Edit::Render(GuiRenderer* renderer, uint32_t w, uint32_t h)
	{
	...
	int32_t Vector2Edit::GetLabelWidth() const
	{
